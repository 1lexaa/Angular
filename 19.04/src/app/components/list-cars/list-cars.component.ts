...
export class ListCarsComponent implements OnInit {
  ...
  deleteSelectedRows() {
    const selectedCheckboxes = Array.from(document.querySelectorAll<HTMLInputElement>('input[type=checkbox]:checked'));
    const selectedIds = selectedCheckboxes.map(cb => cb.id);
  
    for (let i = 0; i < selectedCheckboxes.length; i++) {
      this.deleteCar(parseInt(selectedCheckboxes[i].id));
    }
  }
